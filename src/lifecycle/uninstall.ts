...
export function buildUninstallActions(root: string, { includeState = false } = {}): ManagedAction[] {
  ...
  actions.push(
    ...
    plannedPreserve(root, ".opennori/current", "current-goal", "The current goal and evidence are preserved unless --include-state is provided."),
    plannedPreserve(root, ".opennori/drafts", "draft-goals", "Draft contracts are preserved unless --include-state is provided."),

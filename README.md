...
architecture decision, and completion decision. It observes `.opennori/` state;
it is not an agent runtime, process log, or replacement for `status` and
`report`.

Agents may publish live activity while working:

```bash
opennori activity start --root . --agent Codex --skill nori-evidence --state verifying --summary "Verifying the current gap"
opennori activity finish --root . --summary "Evidence recorded"
```
...
- `dashboard` starts a loopback visual observation surface backed by events,
  activity, and snapshots. It does not execute agent work or certify completion.
...
Dashboard 展示当前 agent 活动、目标、验收缺口、是否需要用户介入、架构判断和完成判断。它只是 `.opennori/` 状态上的观察面，不是 agent runtime、过程日志，也不能替代 `status` 和 `report`。

Agent 可以在工作时发布实时活动信号：

```bash
opennori activity start --root . --agent Codex --skill nori-evidence --state verifying --summary "正在验证当前缺口"
opennori activity finish --root . --summary "证据已记录"
```
...
- `dashboard` 启动 loopback 本地视觉观察面，读取事件、活动和状态投影。它不执行 agent 工作，也不认证完成。
- `activity` 命令只发布 agent 实时活动信号。活动信号不是证据。

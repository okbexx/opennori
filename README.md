...
当一个目标被确信完成后，CLI 里的 `resume`、`status`、`next`、`report` 等命令和上下文导出数据中都会包含 `next_recommendation.candidate_goals`。这是自动生成的一组小型后续卡片，为下一个 Nori 契约提供合理的起点：每一个候选提案均明确指出了建议的目标、用户价值、验收方向以及偏离风险。
...
- 上下文导出仅为其他审查和报告工具提供当前的契约画像与状态数据，审查工具不可反向干预代理的自治运行。

...
For local OpenNori development from a checkout, preview the Plugin cache update
and then confirm it:
...
`Enhanced Discovery checked`, the draft's Acceptance Basis records
`source: autogoal` and `mode: enhanced`, and `opennori status` / reports show
the discovery coverage summary. If those signals are missing, the agent should
revise the draft basis instead of asking for approval.
...
Loop Engineer is a packaged Skill behavior, not a new CLI command. It reads the
current `agent_next` route from `opennori resume/status`, chooses the correct
focused OpenNori Skill, advances one acceptance loop, records activity or
evidence only through the existing state layer, and reports Goal, Current gap,
Loop type, Action taken, Evidence, Decision, Need user, and Next. It must stop
at AC approval, architecture confirmation, waiver, install confirmation, report
acceptance, or any other user decision boundary.
...
dossier. `contract.json` is the goal-level source of truth, `ledger.json` is the
deterministic evidence/status ledger, goal `README.md` is the review surface,
and each `criteria/<AC-id>/` directory keeps that AC's criterion, status,
...
opennori discover --goal "Ship a settings page" --questions '<skill-prepared-questions>' --root .
opennori brainstorm --idea "Explore this goal" --candidates '<skill-prepared-candidates>' --root .
opennori draft --brief <skill-prepared-brief.json> --root .
...
如果您是在本地拉取了 OpenNori 源码并进行二次开发，先预览插件缓存更新，再确认执行：
...
用户能从三个地方确认增强模式确实被使用：agent 回复里出现 `Enhanced Discovery checked`，draft 的 Acceptance Basis 记录 `source: autogoal` 和 `mode: enhanced`，以及 `opennori status` / report 展示 discovery coverage summary。如果这些信号缺失，agent 应先修正 draft basis，而不是要求用户 approve。
...
Loop Engineer 是打包 Skill 行为，不是新的 CLI 命令。它读取
`opennori resume/status` 返回的 `agent_next`，判断当前缺口属于验收、架构、实现、证据、Profile、报告、健康恢复还是下一份契约，然后调用正确的 OpenNori 子 Skill，只推进一轮 acceptance loop。它的回复应包含 Goal、Current gap、Loop type、Action taken、Evidence、Decision、Need user 和 Next。
...
opennori plugin sync                                                   # 本地开发/恢复时预览刷新 Codex Plugin 缓存
opennori plugin sync --local --confirm                                 # 源码 checkout 开发时应用本机 packaged Skills 更新
...
opennori discover --goal "Ship a settings page" --questions '<skill-prepared-questions>' --root .   # 保存 Skill 准备的验收问题来源
opennori brainstorm --idea "探索当前业务目标下的架构偏离" --candidates '<skill-prepared-candidates>' --root . # 保存 Skill 准备的候选方向

...
should first explain its understanding of every AC: where the user enters, what
...
The adoption reply should still include AC Interpretation Review before
approval. Conversation notes are not enough; the user must be able to catch
whether the agent understood each AC correctly.
...
接管已有讨论时同样需要“我的理解”环节。聊天记录不是批准依据；用户需要看到 agent 对每条 AC 的理解是否正确，再决定 approve 或 revise。
...
请求 approve 前，Nori 应逐条说明它如何理解这些 AC。用户批准的是 Nori Contract 以及 agent 对每条 AC 的理解，而不是只批准一组看起来像 AC 的句子。

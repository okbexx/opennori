...
- "add AC-14", "补一条 AC", "还缺一个设置 AC", or AC Review Loop reveals a missing draft acceptance boundary -> hand off to `nori-acceptance`; the child Skill should use `opennori criterion add --from-draft --goal <goal-id>` instead of patching `contract.json`, `ledger.json`, or criterion dossier files by hand.
...
- "验收标准用中文", "用中文写 Nori Contract", "write the AC in English", or any explicit Contract language request -> carry that preference to `nori-acceptance`; the child Skill records it as Contract presentation, not as Product AC. Also carry this language into Nori Profile and project Architecture Profile creation so user-readable profile values match the user's review language.
- "profile 为什么是英文", "Profile 内容应该中文", "architecture profile language is wrong", or a Chinese prompt produced English profile content -> route to `nori-capability-profile` for Nori Profile items or `nori-architecture-brainstorm` for project Architecture Profiles. The CLI should not auto-translate old assets; the agent should revise or recreate the user-readable profile content in the current `presentation.language` or explicit user language.
...
- Do not generate Nori Profile item names/purposes or project Architecture Profile titles, summaries, checks, decisions, reasons, and review text in English when the user prompt or current contract is Chinese, unless the user explicitly requested English. Protocol keys and stable ids stay English; user-readable values follow `presentation.language` or the user's explicit language.

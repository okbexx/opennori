...
Autogoal enhanced mode is not a CLI mode or new artifact. When the user asks for enhanced autogoal, self-grill, "agent grill yourself", or expects all usage scenarios from a rough idea such as a todolist, `nori-autogoal` must perform Enhanced Discovery first: internally expand scenarios, states, data rules, failures, recovery paths, UX expectations, persistence, review methods, assumptions, and out-of-scope boundaries; then show a compact coverage summary and only critical questions that change completion meaning before producing the standard draft.
